...
function pk_shortcode_register()
{
    ...
    $list = array(
        'music' => array('name' => __('Music Player', PUOCK), 'content' => __('Enter link URL', PUOCK)),
        'pre' => array('name' => __('Code Embed', PUOCK), 'content' => __('Enter code', PUOCK)),
        'reply' => array('name' => __('Visible after reply', PUOCK), 'content' => __('Enter content', PUOCK)),
        'login' => array('name' => __('Visible after login', PUOCK), 'content' => __('Enter content', PUOCK)),
        'github' => array('name' => __('Github Repository Card', PUOCK), 'content' => 'Licoy/wordpress-theme-puock'),
        'login_email' => array('name' => __('Visible after login and email verification', PUOCK), 'content' => __('Enter content', PUOCK)),
        'video' => array('name' => __('Video Player', PUOCK), 'attr' => array(
            'url'=>'example.com/test.mp4',
            'autoplay' => false, 'type' => 'auto',
            'pic' => '', 'class' => ''
        )),
        'download' => array('name' => __('File Download', PUOCK), 'content' => __('File URL', PUOCK), 'attr' => array(
            'file' => 'xxx.zip', 'size' => '12MB'
        )),
        'password' => array('name' => __('Visible after password', PUOCK), 'content' => __('Enter content', PUOCK), 'attr' => array(
            'pass' => '123456', 'desc' => __('Enter password to view', PUOCK),
        )),
        'collapse' => array('name' => __('Collapsible Panel', PUOCK), 'content' => __('Enter content', PUOCK), 'attr' => array(
            'title' => 'title'
        )),
    );
    foreach ($shortCodeColors as $sc_tips) {
        $list['t-' . $sc_tips] = array(
            'name' => __('Alert Box', PUOCK) . ' ' . $sc_tips,
            'attr' => array(
                'icon' => ''
            ),
            'content' => __('Enter content', PUOCK)
        );
    }
    return $list;
}
...
function pk_sc_video($attr, $content = null)
{
    ...
    if (empty($url) && empty($href)) {
        return sc_tips(array('outline'=>true), '<span class="c-sub fs14">' . __('Video warning: Play URL cannot be empty', PUOCK) . '</span>', 't-warning');
    }
    if (empty($url)) {
        $url = $href;
    }
...
{
    if (empty($content)) {
        return sc_tips(array('outline' => true), '<span class="c-sub fs14">' . __('Audio warning: Play URL cannot be empty', PUOCK) . '</span>', 't-warning');
    }
...
function pk_download($attr, $content = null)
{
    ...
    return "<div class=\"p-block p-down-box\">
        <div class='mb15'><i class='fa fa-file-zipper'></i>&nbsp;<span>" . __('File name:', PUOCK) . " $filename</span></div>
        <div class='mb15'><i class='fa fa-download'></i>&nbsp;<span>" . __('File size:', PUOCK) . " $size</span></div>
        <div class='mb15'><i class='fa-regular fa-bell'></i>&nbsp;<span>" . __('Download notice:', PUOCK) . " $down_tips</span></div>
        <div><i class='fa fa-link'></i><span>" . __('Download URL:', PUOCK) . " $content</span></div>
    </div>";
}
...
function pk_reply_read($attr, $content = null)
{
    ...
    $msg = sc_tips(array('outline' => true), "<span class='c-sub fs14'><i class='fa-regular fa-eye'></i>&nbsp;" . __('Hidden content here. Submit a comment and refresh after approval to view!', PUOCK) . "</span>", 't-primary');
    if ($user_id > 0) {
...
{
    $msg = sc_tips(array('outline' => true), "<span class='c-sub fs14'><i class='fa-regular fa-eye'></i>&nbsp;" . __('Hidden content here. Log in to view!', PUOCK) . "</span>", 't-primary');
...
function pk_login_email_read($attr, $content = null)
{
    ...
    return sc_tips(array('outline' => true), "<span class='c-sub fs14'><i class='fa-regular fa-eye'></i>&nbsp;" . __('Hidden content. Login and verify email to view!', PUOCK) . "</span>", 't-primary');
}
...
function pk_password_read($attr, $content = null)
{
    ...
    $error = '';
    if (empty(trim($desc ?? ''))) {
        $desc = __('Hidden content. Enter correct password to view!', PUOCK);
    }
    ...
    if (isset($_REQUEST['pass'])) {
        if ($_REQUEST['pass'] == $pass) {
            ...
            $info .= "<p class='fs14 text-danger'><i class='fa-solid fa-triangle-exclamation'></i>&nbsp;" . __('Wrong password. Please check and try again', PUOCK) . "</p>";
        }
    }
    $out .= "<div class='alert alert-primary alert-outline'>{$info}"
        ...
        . "<input type=\"password\" placeholder=\"" . esc_attr__('Enter password', PUOCK) . "\" required class=\"form-control form-control-sm\" name=\"pass\"/>"
        . "</div><div class=\"col-4 col-md-2 pl-0\"><button class=\"btn btn-sm btn-primary w-100\">" . __('View Now', PUOCK) . "</button></div></div></form>"
        . "</div>";
    return $out;
}
