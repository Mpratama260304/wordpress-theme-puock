...
function pk_comment_author_view($text, $comment)
{
    ...
    if ($author_view && !$is_author) {
        $text = apply_filters('pk_comment_author_view', '<div class="fs12 c-sub"><i class="fa fa-lock"></i> ' . __('Comment visible to author only', PUOCK) . '</div>');
    }
    return $text;
}
            ...
            <div class="t-sm c-sub">
                ...
                <?php if ($comment->comment_approved == '1' && (!$author_cat_comment || $is_author)) : ?>
                    <a id="comment-reply-<?php comment_ID() ?>" data-id="<?php comment_ID() ?>"
                       ...
                       href="javascript:void(0)" title="<?php esc_attr_e('Reply to this comment', PUOCK); ?>"><i class="fa fa-share-from-square"></i>
                        <span class="comment-reply-text"><?php _e('Reply', PUOCK); ?></span></a>
                <?php endif; ?>
            </div>
        </div>
    </div>
    <div class="content">
        <div class="content-text t-md mt10 puock-text" <?php if (!pk_open_show_comment_avatar()) {
            ...
            <?php if ($comment->comment_approved == '0') : ?>
                <p class="c-sub mt-1"><i class="fa fa-warning mr-1"></i><?php _e('Your comment is awaiting moderation!', PUOCK); ?></p>
            <?php endif; ?>
