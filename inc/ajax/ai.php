...
function pk_ajax_ai_ask()
{
    if (!pk_is_checked('ai_chat_enable')) {
        wp_die('<code>' . __('AI Assistant not enabled', PUOCK) . '</code>');
    }
    $uid = get_current_user_id();
    if (!$uid && !pk_is_checked('ai_guest_use')) {
        wp_die('<code>' . __('Guests are not allowed to use AI Assistant', PUOCK) . '</code>');
    }
    ...
    $model = $body['model'] ?? '';
    if (empty($text)) {
        wp_die('<code>' . __('Please enter a description', PUOCK) . '</code>');
    }
    ...
    if (empty($openai_api_key)) {
        wp_die('<code>' . __('Please configure OpenAI API Key first', PUOCK) . '</code>');
    }
    ...
    if ($use_img_mode) {
        if (!pk_is_checked('ai_draw_dall_e')) {
            wp_die('<code>' . __('AI Drawing is not enabled', PUOCK) . '</code>');
        }
        try {
            ...
            if (!$res) {
                wp_die('<code>' . __('AI Drawing failed: Response parse error', PUOCK) . '</code>');
            }
            $answer = $res->data[0]->url;
            wp_die('![img](' . $answer . ')');
        } catch (Exception $e) {
            wp_die('<code>' . __('AI Drawing failed:', PUOCK) . ' ' . $e->getMessage() . '</code>');
        }
    }
    ...
    try {
        if ($use_stream) {
            ...
            $res = json_decode($chat_res);
            if (!$res) {
                wp_die('<code>' . __('AI Q&A parse error:', PUOCK) . ' ' . $chat_res . '</code>');
            }
            if (isset($res->error)) {
                wp_die('<code>' . __('AI Q&A error:', PUOCK) . ' ' . $res->error . '</code>');
            }
        ...
        wp_die('<code>' . __('AI Q&A error:', PUOCK) . ' ' . $e->getMessage() . '</code>');
    }
}
