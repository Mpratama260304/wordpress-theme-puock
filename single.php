            ...
            <div id="post-main"
                ...
                <div class="p-block">
                        ...
                        <div>
                            <?php if (!pk_is_checked('hide_post_views')): ?>
                                <div class="option puock-bg ta3 t-sm mr-1"><i
                                    ...
                                    <span id="post-views"><?php pk_get_post_views(); ?></span><span><?php _e(' Views', PUOCK) ?></span>
                                </div>
                            <?php endif; ?>
                            ...
                            <?php if (is_user_logged_in() && current_user_can('edit_post', $post->ID)): ?>
                                <a target="_blank" href="<?php echo get_edit_post_link() ?>">
                                    <div class="option puock-bg ta3 t-sm mr-1"><i
                                                class="fa-regular fa-pen-to-square mr-1"></i><?php _e('Edit', PUOCK) ?>
                                    </div>
                                </a>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="mt20 entry-content-box">
                        ...
                        <?php endif; ?>
                        <div class="t-separator c-sub t-sm mt30"><?php _e('END', PUOCK) ?></div>
                        <?php if (pk_is_checked('post_foot_qrcode_open')): ?>
                            <div class="post-foot-qrcode">
                                <div class="title"><?php echo pk_get_option('post_foot_qrcode_title', __('No description', PUOCK)) ?></div>
                        ...
                        <div class="footer-info puock-text mt20">
                            ...
                            <div class="p-flex-sbc mt20 t-sm">
                                <div>
                                    <span><?php _e('Posted to: ', PUOCK) ?></span><?php echo get_post_category_link_exec(true) ?>
                                </div>
                                <div>
                    ...
                    <div class="p-block p-flex-sc">
                        ...
                        <div class="puock-text t-md">
                            ...
                            if (empty($origin_author)): ?>
                                <div>
                                    <span class="font-weight-bold"><?php _e('Copyright Notice: ', PUOCK) ?></span><span><?php _e('Original article from this site, by', PUOCK) ?></span>
                                    <a class="a-link"
                                       href="<?php global $authordata;
                                       if ($authordata) {
                                           echo get_author_posts_url($authordata->ID,
                                               $authordata->user_nicename);
                                       } ?>"><?php the_author() ?> </a><?php _e('on ', PUOCK) ?><?php the_date('Y-m-d') ?><?php
                                    _e(', ', PUOCK) ?><?php _e('total ', PUOCK) ?><?php echo count_words() ?><?php _e(' words.', PUOCK) ?>
                                </div>
                                <div class="mt-2">
                                    <span class="font-weight-bold c-sub"><?php _e('Reproduction Note: ', PUOCK) ?></span><span
                                            class="c-sub"><?php echo pk_get_option('post_reprint_note', pk_get_option('footer_copyright')) ?></span>
                                </div>
                            <?php else: ?>
                                <div>
                                    <span class="font-weight-bold"><?php _e('Copyright Notice: ', PUOCK) ?></span><?php _e('This article was published on ', PUOCK) ?><?php the_date('Y-m-d')
                                    ?><?php _e(' reprinted from ', PUOCK) ?><a target="_blank"
                                                                  href="<?php echo get_post_meta(get_the_ID(), 'origin_url', true) ?>"
                                                                  class="a-link" rel="nofollow"><?php
                                        echo $origin_author ?></a><?php _e(', total ', PUOCK) ?><?php echo count_words('') ?><?php _e(' words.', PUOCK) ?>
                                </div>
                                <div class="mt-2">
                                    <span class="font-weight-bold c-sub"><?php _e('Reprint tips: ', PUOCK) ?></span><span
                                            class="c-sub"><?php _e('This article is not original to this site. If you need to reprint, please contact the original author for reprint authorization.', PUOCK) ?></span>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
